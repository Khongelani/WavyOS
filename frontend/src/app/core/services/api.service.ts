...
export interface WebIntelligenceReport {
  ...
  jse_ticker?: string;
  latest_stock_data?: StockData;
  news_articles: NewsArticle[];
  sens_announcements: NewsArticle[];
  leadership_mentions: { name: string; role?: string; confidence?: string }[];
  linkedin_company_url?: string;
  leadership_page_url?: string;
...
export interface ContactDiscovery {
  ...
  recommended_departments: { department: string; reason?: string }[];
  linkedin_search_urls: LinkedInUrl[];
...
export interface IntelligenceData {
  intelligence: WebIntelligenceReport | null;
  discovery: ContactDiscovery | null;
}

export interface IntelligenceStatus {
  status: 'pending' | 'running' | 'complete' | 'failed' | 'not_scanned';
  completed_at?: string;
  started_at?: string;
  error?: string;
}
...
export class ApiService {
  ...
  // Web Intelligence
  getIntelligence(companyId: number): Observable<IntelligenceData> {
    return this.http.get<IntelligenceData>(`${this.base}/companies/${companyId}/intelligence`);
  }

  getIntelligenceStatus(companyId: number): Observable<IntelligenceStatus> {
    return this.http.get<IntelligenceStatus>(`${this.base}/companies/${companyId}/intelligence/status`);
  }

  triggerIntelligenceScan(companyId: number): Observable<{ message: string; status: string }> {
    return this.http.post<{ message: string; status: string }>(`${this.base}/companies/${companyId}/intelligence/scan`, {});
  }

  retryIntelligenceScan(companyId: number): Observable<{ message: string }> {
    return this.http.post<{ message: string }>(`${this.base}/companies/${companyId}/intelligence/scan/retry`, {});
  }
}
