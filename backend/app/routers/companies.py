...
async def list_companies(
    ...
    q = select(Company).options(selectinload(Company.pipeline_stage))
    if search:
...
async def create_company(
    ...
    result = await db.execute(
        select(Company).options(selectinload(Company.pipeline_stage)).where(Company.id == company.id)
    )
    company = result.scalar_one()

    # Auto-trigger web intelligence scan in background
    if settings.ENABLE_WEB_INTELLIGENCE:
        from app import web_intelligence_service
        background_tasks.add_task(web_intelligence_service.run_full_scan, company.id)
...
async def get_company(
    ...
    result = await db.execute(
        select(Company).options(selectinload(Company.pipeline_stage)).where(Company.id == company_id)
    )
...
async def update_company(
    ...
    # Re-fetch to get refreshed relationship after commit
    result = await db.execute(
        select(Company).options(selectinload(Company.pipeline_stage)).where(Company.id == company_id)
    )
    company = result.scalar_one()
